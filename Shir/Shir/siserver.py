...
class SystemMonitor:
    ...
    def get_size(bytes_size, suffix="B"):
        # ממיר גדלי זיכרון לפורמט שהוא קריא (KB, MB וכו)
        # הפונקציה הזו מקבלת מספר של בתים (Bytes)
        # וממירה אותו ליחידות יותר נוחות לקריאה כמו KB, MB, GB
        # לדוגמה: 1000000 Bytes יהפכו ל-1MB
        factor = 1024
        # עוברים על יחידות זיכרון מהקטן לגדול
        for unit in ["", "K", "M", "G", "T", "P"]:
            # אם הערך קטן מ-1024 אין צורך להמיר אותו ליחידה גדולה יותר
            # כי הוא עדיין מספיק קטן כדי להיות מוצג בצורה ברורה
    ...
    def get_dashboard_info():
        uname = platform.uname() # מידע על מערכת ההפעלה והמחשב
        svmem = psutil.virtual_memory() # מידע על זיכרון RAM
        cpu_brand = cpuinfo.get_cpu_info().get('brand_raw', 'Unknown CPU')
        ip_addr = socket.gethostbyname(socket.gethostname())  # כתובת IP מקומית של המחשב ברשת
        mac_addr = ':'.join(re.findall('..', '%012x' % uuid.getnode())) # כתובת MAC שהיא מזהה ייחודי של כרטיס הרשת
    ...
    @staticmethod
    # הפונקציה סורקת את כל התהליכים שרצים במחשב ומחזירה מידע עליהם
    # כל תהליך הוא תוכנה או שירות שפועל ברקע
    def get_afk_info() -> str:
        # שלב ראשון מאפס מדידת CPU כדי לקבל נתונים מדויקים יותר
        # כי psutil מחשבת שימוש לפי הפרש בין שתי מדידות
        for proc in psutil.process_iter():
            try:
                proc.cpu_percent(interval=None)
            except:
                pass
        # המתנה קצרה כדי לאפשר למערכת לאסוף נתוני שימוש אמיתיים
        time.sleep(1.0)
        processes = []
        cpu_count = psutil.cpu_count() or 1  # מספר ליבות במעבד
        # מעבר על כל התהליכים במערכת
        for process in psutil.process_iter():
            with process.oneshot():
                try:
                    ...
                    cpu_usage = round(process.cpu_percent(interval=None) / cpu_count, 1)  # שימוש CPU מחושב לפי מספר ליבות
    ...
    def get_net_connections() -> str:
        # הפונקציה מחזירה את כל החיבורים הפעילים לרשת
        # אפשר לראות לאיזה כתובות המחשב מתקשר
        connections = []
    ...
    def get_persistence_info() -> str:
        # הפונקציה בודקת אילו תוכנות מוגדרות לעלות אוטומטית עם המחשב
        # זה נקרא Persistence וזה משהו חשוב באבטחת מידע
        # כי גם תוכנות זדוניות משתמשות בזה כדי להישאר פעילות אחרי ריסט
        startup_items = []
        keys_to_check = [
    ...
    def get_anomalies() -> str:
       # הפונקציה הזאת מחפשת התנהגות חריגה במחשב
        anomalies = []
        sus_ports = [4444, 1337, 666, 8888, 9999, 4432] #פורטים שנחשבים חשודים בהקשרים מסוימים
       #מקומות במחשב שיכולים להימצא בהם דברים מזיקים או חשודים
    ...
    def get_main_parent(pid):
        # לפעמים תהליך יוצר תתי תהליכים עם אותו שם
        # הפונקציה הזאת עולה למעלה בעץ התהליכים
        # כדי למצוא את התהליך הראשי באמת
        try:
    ...
    def terminate_process(pid_str: str) -> str:
        # מאפשר לסגור תהליך או עץ שלם של תהליכים
        # שימושי כשצריך לעצור תוכנה לגמרי
        parts = pid_str.split(":")
    ...
    def suspend_process(pid_str: str) -> str:
        # עוצר תהליך בלי לסגור אותו לגמרי
        try:
            pid = int(pid_str)
    ...
    def resume_process(pid_str: str) -> str:
        # מחזיר תהליך לפעולה אחרי שהוא הוקפא
        try:
            pid = int(pid_str)
...
class ClientHandler(Thread):
    ...
    def run(self):
        # כל לקוח רץ על Thread משלו
        # כדי שהשרת יוכל לשרת כמה מחשבים במקביל
        try:
            ...
            while True:
                ...
                if command in self.command_map:
                    ...
                    reply = SystemMonitor.suspend_process(param)
                elif command == "RESUME" and param:
                    reply = SystemMonitor.resume_process(param)
                else:
...
if __name__ == "__main__":
    CERT_FILE = r"C:\Users\Gal\PycharmProjects\YD_ASH_2025\Shir\server.crt"
    KEY_FILE = r"C:\Users\Gal\PycharmProjects\YD_ASH_2025\Shir\server.key"
